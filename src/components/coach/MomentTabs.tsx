        ...
        {TABS.map(t => {
          ...
          return (
            <button
              ...
              className={cn(
                'h-14 w-full flex items-center gap-2 rounded-lg px-3 py-2 text-left transition-all',
                active
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className={cn('text-xs font-bold leading-tight truncate', active ? 'text-foreground' : 'text-muted-foreground')}>
                  {t.label}
                </div>
                <div className="text-[10px] text-muted-foreground leading-tight truncate">{t.sub}</div>
              </div>
            </button>
          );
        })}
