        ...
        tables: [
          {
            ...
            rows: budgetVsActual.slice(0, 12).map((b) => [
              ...
              money(b.budget),
              { text: money(b.actual), tone: (b.actual > b.budget ? 'red' : 'navy') as Tone, bold: true },
              { text: money(b.budget - b.actual), tone: (b.budget - b.actual < 0 ? 'red' : 'green') as Tone, bold: true },
            ]),
          ...
          },
          {
            ...
            rows: monthlyCashflow.slice(-8).map((m) => [
              ...
              { text: money(m.savings), tone: (m.savings >= 0 ? 'green' : 'red') as Tone, bold: true },
            ]),
        ...
        tables: [
          {
            ...
            rows: accountRows.map((a) => [
              ...
              { text: money(a.balance), tone: (a.balance < 0 ? 'red' : 'green') as Tone, bold: true },
            ]),
      ...
      tables: [
        {
          ...
          rows: incomeVsExpenses.byMonth.slice(-10).map((m) => [
            ...
            { text: money(m.income), tone: 'green' as const },
            { text: money(m.expenses), tone: 'red' as const },
            { text: money(m.income - m.expenses), tone: (m.income - m.expenses >= 0 ? 'green' : 'red') as Tone, bold: true },
          ]),
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
              ...
              <TooltipTrigger asChild>
                <span>
                  <TabsTrigger value={value} className="gap-1.5">
                    <TabIcon className={cn("h-4 w-4 sm:text-current", color)} />
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={280}>
                  <AreaChart data={dailySpending}>
                    ...
                    <Tooltip formatter={(v: number, name: string) => [formatCurrency(v), name === 'daily' ? 'Daily' : 'Cumulative']} contentStyle={tooltipStyle} itemStyle={{ color: 'hsl(var(--foreground))' }} labelStyle={{ color: 'hsl(var(--foreground))' }} />
                    <Legend />
                ...
                <>
                  <ResponsiveContainer width="100%" height={Math.max(300, budgetVsActual.length * 50 + 60)}>
                    <BarChart data={budgetVsActual} layout="vertical" margin={{ left: 20, right: 20 }}>
                      ...
                      <Tooltip cursor={{ fill: 'transparent' }} formatter={(v: number, name: string) => [formatCurrency(v), name === 'budget' ? 'Budget' : 'Actual']} contentStyle={tooltipStyle} itemStyle={{ color: 'hsl(var(--foreground))' }} labelStyle={{ color: 'hsl(var(--foreground))' }} />
                      <Legend />
        ...
        <TabsContent value="wealth">
          <div className="flex gap-1 mb-4 rounded-lg bg-muted p-1 w-fit">
            <button onClick={() => setWealthSub('networth')} className={cn('px-3 py-1.5 text-xs font-medium rounded-md transition-colors', wealthSub === 'networth' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground')}>
              <TrendingUp className="h-3.5 w-3.5 inline mr-1.5" />Net Worth
            </button>
            <button onClick={() => setWealthSub('trends')} className={cn('px-3 py-1.5 text-xs font-medium rounded-md transition-colors', wealthSub === 'trends' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground')}>
              <LineChartIcon className="h-3.5 w-3.5 inline mr-1.5" />Trends
            </button>
          </div>
          {wealthSub === 'networth' && (
          <>
