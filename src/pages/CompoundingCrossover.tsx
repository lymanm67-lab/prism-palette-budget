...
export default function CompoundingCrossover() {
  ...
  const reportProps = {
    label: eff.label,
    balance: eff.balance,
    contributions: eff.contributions,
    returnPct: state.returnPct,
    contributionGrowthPct: state.contributionGrowthPct,
    debtRedirectAnnual: eff.redirect,
    debtRedirectStartYear: state.debtRedirectStartYear,
    crossoverYear: active.crossoverYear ?? null,
    crossoverPortfolio: active.crossoverPortfolio ?? null,
    rows: active.rows,
    chartData,
    byScenario,
    household,
  ...
  return (
    <div className="min-h-screen bg-background text-foreground print-root">
      ...
      @media screen {
        ...
        .print-report.preview-open { display: block; }
      }`}</style>

      {/* Print / preview report */}
      <div className={`print-report ${previewOpen ? 'preview-open' : ''}`}>
        {previewOpen && (
          <div className="no-print sticky top-0 z-40 flex flex-wrap items-center justify-between gap-3 border-b border-border bg-card px-4 py-3">
            <div>
              <div className="text-sm font-semibold">Print preview — The Compounding Crossover™</div>
              <div className="text-xs text-muted-foreground">
                4 pages · US Letter · {eff.label} @ {state.returnPct}%
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => setPreviewOpen(false)}>
                <X className="mr-1.5 h-3.5 w-3.5" /> Close preview
              </Button>
        ...
        <div className={previewOpen ? 'overflow-x-auto py-6' : ''}>
          <CrossoverReport {...reportProps} />
        </div>
      </div>

      <div className={`screen-only mx-auto max-w-6xl px-4 py-8 ${previewOpen ? 'hidden' : ''}`}>
              ...
              <label className="mb-3 block text-xs font-semibold text-muted-foreground">
                ...
                <div className="relative mt-1">
                  ...
                  <NumberInput
                    ...
                    onChange={(n) => set({ annualContributions: n })}
                    className="w-full rounded-md border bg-background py-2 pl-7 pr-3 text-sm font-medium text-foreground"
                  />
                </div>
              </label>
              <label className="mb-3 block text-xs font-semibold text-muted-foreground">
                ...
                <div className="relative mt-1">
                  <NumberInput
                    value={state.contributionGrowthPct}
                    decimals={1}
                    onChange={(n) => set({ contributionGrowthPct: n })}
              ...
              <label className="mb-3 block text-xs font-semibold text-muted-foreground">
                ...
                <div className="relative mt-1">
                  ...
                  <NumberInput
                    ...
                    onChange={(n) => set({ debtRedirectAnnual: n })}
                    className="w-full rounded-md border bg-background py-2 pl-7 pr-3 text-sm font-medium text-foreground"
                  />
                </div>
              </label>
                  ...
                  <>
                    <label className="mt-3 block text-xs font-semibold text-muted-foreground">
                      ...
                      <NumberInput
                        ...
                        onChange={(n) => set({ kateriBalance: n })}
                    ...
                    <label className="mt-3 block text-xs font-semibold text-muted-foreground">
                      ...
                      <NumberInput
                        ...
                        onChange={(n) => set({ kateriContributions: n })}
                        className="mt-1 w-full rounded-md border px-3 py-2 text-sm font-medium bg-background text-foreground"
                      />
                    </label>
