...
function WeeklyReviewSection() {
  ...
  // The measurable parts are read from the trade record rather than typed, so a
  // review cannot flatter the week it is reviewing.
  const measured = useMemo(() => {
    const inWeek = trades.filter(
      (t) => t.status === 'CLOSED' && (t.exit_date ?? '') >= weekStart && (t.exit_date ?? '') <= addDays(weekStart, 6),
    );
