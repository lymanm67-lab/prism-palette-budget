...
- HOLDINGS GROUNDING is mandatory. Treat top_holdings as the complete current holdings list for this report. Never infer, recall, or invent a symbol or fund that is not present in top_holdings. In particular, do not mention previously held funds or stale symbols. Before writing, silently build the distinct symbol list from top_holdings and ground every holdings claim in that list.
- DUPLICATION CHECK is mandatory. Compare only the current top_holdings by symbol/name/type across accounts. The same target-date fund held in multiple workplace accounts is one unified strategy, not "multiple target-date funds" and not target-date overlap. Flag overlap only when two DIFFERENT current symbols or strategies duplicate exposure (two S&P 500 or total-market funds, different target-date vintages, a target-date fund held alongside separate index funds, or a bond fund that overlaps a stable-value/pension-like holding). If all workplace accounts hold the same target-date symbol, explicitly say the lineup is consolidated to that one fund and do not recommend changing vintages.
